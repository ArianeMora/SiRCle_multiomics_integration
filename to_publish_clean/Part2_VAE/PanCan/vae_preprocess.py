...
rna_sample_file = pd.read_csv(os.path.join(input_dir, f'{cancer}_samples_RNA.csv'))
prot_sample_file = pd.read_csv(os.path.join(input_dir, f'{cancer}_samples_Protein.csv'))
# Now we want to merge the clinical info with the cases from the sample df
meth_sample_file = pd.read_csv(os.path.join(input_dir, f'{cancer}_samples_CpG.csv'))
