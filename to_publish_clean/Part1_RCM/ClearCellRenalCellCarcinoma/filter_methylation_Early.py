...
dcpg = os.path.join(input_dir, f'{cancer}_filtered_DMC_CpG_Early.csv')
...
cpg_samples = cpg_samples[cpg_samples['Stage'] == 'Early']
tumor_samples = list(cpg_samples[cpg_samples['CondID'] == 1]['Sample'].values)
normal_samples = list(cpg_samples[cpg_samples['CondID'] == 0]['Sample'].values)
...
filtered_dcpg.to_csv(os.path.join(input_dir, f'{cancer}_filtered_DCpG_Early.csv'), index=False)
