...
pairedPatientDA <- function(data_file, sample_file, output_file, paired=FALSE) {
...
pairedPatientDMC <- function(data_file, sample_file, output_file, paired=FALSE) {
  ...
  all_cpg_df <- cpg_raw[rownames(fit2_adj), c('Locus', 'chr', 'pos', 'Relation_to_Island', 'UCSC_RefGene_Group', 'ensembl_gene_id', 'external_gene_name', 'hgnc_symbol', 'entrezgene_id')]
