...
scivae <<- import("scivae")
#conda_install('omics', "mofapy")
...
runfactorization <- function(folder,file.names,num.factors,sep=" ",filtering="none"){
  ...
  timings <- list() # Make a list of time to complete
  ...
  # VAE
  start.time <- Sys.time()
  metagenes_scivae<- list()
  driver_dfs <- list()
  n_facs <- num.factors
  for(j in 1:length(omics)){
    # Need to create a vae
    df <- as.matrix(omics[[j]])
    vae <- scivae$VAE(df, df, rownames(omics[[j]]), paste0("config_", n_facs, ".json"), 'vae_rcm_c', config_as_str=T)
    # Set the number of latent nodes in the vae
    vae$latent_config$num_nodes <- as.integer(num.factors)
    # Run the training
    vae$encode('default', epochs=as.integer(100))
    data <- vae$encode_new_data(df, encoding_type="z", scale=T)
    rownames(data) <- rownames(omics[[j]])
    metagenes_scivae[[j]] <- data
    colnames(metagenes_scivae[[j]])<-1:num.factors
    n <- 25 #as.integer(length(data)/4)
    # Maybe remove?
    omics_drivers <- c()
    for (i in 1:num.factors) {
      # Get the top N genes from each omics for each latent node
      omics_drivers <- append(omics_drivers, rownames(data[order(data[, i], decreasing = F),])[1:n])
      omics_drivers <- append(omics_drivers, rownames(data[order(data[, i], decreasing = T),])[1:n])
    }
    # Just pick the top 20% of most variable genes i.e. make features 20% of the available test data size
    data_variance <- rowVars(df)
    omics_drivers <- rownames(data[order(data_variance, decreasing = F),])[1:0.2*length(colnames(df))]
    colnames(metagenes_scivae[[j]])<-1:num.factors
    rownames(metagenes_scivae[[j]])<-rownames(omics[[j]])
    # Add the driver genes to the driver dfs, make sure the columns are aligned
    driver_dfs[[j]] <- df[omics_drivers, colnames(omics[[1]])]
  }

  feature_df <- do.call("rbind", driver_dfs)
  # Now pick the top ones and build a dataframe only with the top genes based on the VAE
  df <- t(feature_df)
  df <- as.matrix(df)
  vae <- scivae$VAE(df, df, rownames(feature_df), paste0("config_", n_facs, ".json"), 'vae_rcm_c', config_as_str=T)
  vae$encode('default', epochs=as.integer(100), early_stop=T)
  data <- vae$encode_new_data(df, encoding_type="z", scale=T)
  rownames(data) <- rownames(df)
  factors_scivae<-as.matrix(data)

  factorizations[[t]]<-list(factors_scivae, metagenes_scivae)
  t<-t+1
  method<-c(method,"scivae")

  end.time <- Sys.time()
  timings[["scivae"]] <- end.time - start.time
  ...
  timings[["RGCCA"]] <- end.time - start.time
  ...
  timings[["MCIA"]] <- end.time - start.time
  
  # ###MOFA --> doesn't work with later versions of python ?
  ...
  timings[["iCluster"]] <- end.time - start.time
  ...
  timings[["intNMF"]] <- end.time - start.time
  ...
  timings[["JIVE"]] <- end.time - start.time
  ...
  timings[["tICA"]] <- end.time - start.time
  ...
  write.csv(timings, paste0(folder, 'Time_', num.factors, '.csv'))
